(() => {
    ...
    class ImageDraft {
        ...
        selectSpatial(item) {
            this.items.forEach(candidate => { candidate.spatial = candidate === item; });
        }
        move(item, direction) {
        ...
        payload() {
            ...
            const order = this.items.map(item => {
                ...
                if (item.spatial) spatial = key;
                return key;
            });
            return {files, imageOrder: order.join(','), imageDeleted: this.deleted.join(','), imageRepresentative: representative, imageSpatial: spatial};
        }
    }
    ...
    const draft = new ImageDraft(Array.from(grid.children, node => ({
        key: node.dataset.key, representative: node.dataset.representative === 'true', spatial: node.dataset.spatial === 'true', node
    })));
    ...
    const render = () => {
        ...
        draft.items.forEach((item, index) => {
            ...
            if (item.spatial) {
                const badge = document.createElement('strong'); badge.className = 'image-spatial'; badge.textContent = '모니터링'; meta.append(badge);
            }
            ...
            if (grid.dataset.spatialEnabled === 'true')
                button(item.spatial ? '모니터링 이미지 해제' : '모니터링 이미지로 설정', () => draft.selectSpatial(item.spatial ? null : item));
    ...
    form.addEventListener('formdata', event => {
        ...
        ['imageOrder', 'imageDeleted', 'imageRepresentative', 'imageSpatial'].forEach(key => event.formData.set(key, payload[key]));
    });
    render();
})();
